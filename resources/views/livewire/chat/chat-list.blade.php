<div class="bg-white shadow-lg p-4 rounded-lg">
    <div class="chatlist_header flex items-center justify-between mb-4">
        ...
        <div class="img_container w-12 h-12 rounded-full overflow-hidden">
    ...
    <div class="chatlist_body space-y-4">
        @forelse ($conversations as $conversation)
            <div class="chatlist_item border border-gray-300 rounded-lg p-4 cursor-pointer transition-transform transform hover:scale-105"
                wire:key="{{ $conversation->id }}"
                wire:click="chatUserSelected({{ $conversation->id }}, '{{ $this->getChatUserInstance($conversation, 'id') }}')">
                <div class="flex items-center">
                    <div class="chatlist_img_container w-12 h-12 rounded-full overflow-hidden">
                        <img src="https://ui-avatars.com/api/?name={{ $this->getChatUserInstance($conversation, 'name') }}"
                            alt="{{ $this->getChatUserInstance($conversation, 'name') }}">
                    </div>
                    <div class="chatlist_info ml-4">
                        ...
                        <div class="bottom_row">
                            ...
                            @if ($unreadCount > 0)
                                <div
                                    class="unread_count text-xs font-semibold rounded-full text-white bg-red-500 px-3 py-1">
                                    {{ $unreadCount }}
                                </div>
                            @endif
                        </div>
